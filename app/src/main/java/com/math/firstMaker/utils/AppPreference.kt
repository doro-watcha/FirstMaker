...
import com.math.firstMaker.MainApplication
import com.math.firstMaker.di.ServerType
...
class AppPreference(context: Application) {
    ...
    var uploadingRequestId: Int
        get() = get(KEY.KEY_UPLOADING_REQUEST_ID) ?: -1
        set(value) = set(KEY.KEY_UPLOADING_REQUEST_ID, value)


    fun isUploading() = uploadingRequestId != -1
    fun beginUploading(id: Int) {
        uploadingRequestId = id
    }

    fun endUploading() {
        uploadingRequestId = -1
    }
}
